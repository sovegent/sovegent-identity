 ...
 *   { "1644": { "sovegent": { "v": 1, "proof_hash": "<sha256hex>" } } }
...
export function buildMetadatum(proofHash: string) {
  return { sovegent: { v: 1, proof_hash: proofHash.replace(/^0x/, "").toLowerCase() } };
}
...
export class CardanoAnchorAdapter implements AnchorAdapter {
  ...
  async verify(proofHash: string, anchor: ChainAnchor): Promise<boolean> {
    ...
    for (const row of rows) {
      const entry = row.metadata?.[String(METADATA_LABEL)];
      // legacy "liberproof" key still read for pre-rebrand anchors
      const got = entry?.sovegent?.proof_hash ?? entry?.liberproof?.proof_hash;
